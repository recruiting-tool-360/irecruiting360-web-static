          ...
          try {
            ...
            import('src/pluginSrc/util/AsyncResumeProcessor')
              .then(({ addResumeTask, clearChannelTasks }) => {
                ...
                console.log(`已添加${channelJobList.length}个前程无忧简历任务到队列中`);
              })
              .catch(error => {
...
const recursiveChannelSearch = async (maxIterations, searchRequestData, currentIteration = 1, page = 1, accumulatedData = []) => {
  console.log(`前程无忧递归查询第${currentIteration}次，页码：${page}`);

  try {
    // 查询渠道参数
    console.log(searchRequestData)
    const result = await Job51InfoManager.channelSearchListSimilar(searchRequestData, page);

    // 如果有数据，添加到累积数据中
    if (result && result.dataList && result.dataList.length > 0) {
      accumulatedData.push(...result.dataList);
      console.log(`前程无忧第${page}页获取到${result.dataList.length}条数据，累计${accumulatedData.length}条`);
    ...
    // 返回累积的数据
    console.log(`前程无忧递归查询完成，共获取${accumulatedData.length}条数据`);
    return accumulatedData;
  } catch (error) {
    console.error(`前程无忧递归查询第${currentIteration}次失败:`, error);
    // 发生错误时返回已累积的数据
    return accumulatedData;
  }
};
